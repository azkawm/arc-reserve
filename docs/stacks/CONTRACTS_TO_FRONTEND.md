...
| Safety gates | `safetyState(bool includeCooldown) returns (uint8 failure, uint256 spot, uint256 twap, uint256 nav)`; failure enum `0 None,1 Paused,2 AssetNotActive,3 Matured,4 StaleNAV,5 SpotTwapDeviation,6 MarketNAVDeviation,7 ReserveBelowMinimum,8 Cooldown`. **4, 5 and 6 are reserved and never returned** (5 since D-036, 4 and 6 since D-039); keep them in the mapping so 7 and 8 do not shift. `twap` is always 0 | — |
...
| Policy | `rebalanceCooldown()`, `maxTickShift()`, `lastRebalanceAt()`. **`twapWindow()` and `maxSpotTwapDeviationBps()` are removed (D-036), `maxMarketNAVDeviationBps()` is removed (D-039)** — calling any of the three reverts | — |
...
| 2026-09-12 | **BREAKING** — D-039: NAV gates **nothing** in the market engine. `safetyState` never returns `StaleNAV` (4) or `MarketNAVDeviation` (6) — both are reserved alongside 5, so 7 and 8 do not shift. **`maxMarketNAVDeviationBps()` no longer exists**; a call reverts. **`repositionSafetyState` was never shipped** — the D-038 row above is retracted. `setSafetyPolicy` keeps five parameters but the 4th joins the 1st and 3rd as accepted-and-ignored. `marketPrices()` and `safetyState` still **report** NAV. | Frontend: there is **one safety view again** — gate every keeper button on `safetyState`. Remove `maxMarketNAVDeviationBps()` from the policy card (the call reverts) and remove any "market within X% of NAV" gate row; nothing enforces it. Keep showing NAV and its age — they are still true and still price redemption — but **stop presenting NAV staleness as something that blocks trading or liquidity**, because it no longer does. Concretely: a stale-NAV banner may say "valuation is X days old, redemption quotes may be stale"; it must not say "engine actions unavailable". The failure-code map keeps 4/5/6 as unreachable so the array indices stay put. |
| 2026-09-13 | **CHANGED** — **DemoRegistrar on Arc (5042002) replaced.** The first one could never register anyone (`UnsupportedChain(5042002)`). The new address, `0xF6f77D0bE395df3d04B6E58a4a8fb0c34EC5286d`, is in `deployments/5042002.json`. Same interface: `selfRegister()`, `canSelfRegister(address)`, `isActive()`. | Frontend: read the registrar address per chain from the deployment record. Gate "Verify me" on `canSelfRegister(wallet) && isActive()`. That pair would have hidden the button on the broken registrar, which is the right behaviour on any chain the registrar does not support. |
| 2026-09-12 | **CHANGED** — token ordering is per DEPLOYMENT, not per chain, and it has now flipped once. On the 84532 redeploy `assetIsToken0()` went **true → false**, because mUSD `0x2581c32d…` sorts below SOLAR01 `0xF06c8E6d…` where the previous pair sorted the other way. Ordering moves with deployer nonce, so **any redeploy can flip it again**. Live values now: **false on 31337 and false on 84532** — one mUSD ≈ `+276324` on both. | Frontend: treat ordering as per-connection state read from `assetIsToken0()`; never cache it, never key it off `chainId`, and never infer it from a fixture. Any tick↔price helper takes ordering as a parameter. **The two chains now AGREE, and that is more dangerous than when they differed** — a latent ordering bug will look correct everywhere until the next redeploy flips it back, so cross-chain comparison no longer catches it. Assert both orderings map equivalent ticks to the same human price in tests instead. Note `EngineControls`' hardcoded tick pair is now wrong-signed for 84532 (it was written for `true`); see the known-pitfalls list in `CLAUDE.md`. |
