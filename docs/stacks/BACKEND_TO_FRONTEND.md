...
| 2026-09-13 | **CHANGED** — `/v1/health` `risks` now covers every chain in the database that the serving process can read, not only the chain it indexes, and each row gains `chainId`. New `riskCoverage`: `computed` (chain ids) and `notComputed` (`chainId`, `reason`) for any chain it cannot read. Rows are keyed by `(chainId, assetId)`. | Key risk rows by `(chainId, assetId)` — an assetId is not guaranteed unique across chains. Treat a chain in `notComputed` as UNKNOWN, never as safe: its assets are just as exposed, this process simply cannot see them. A chain also lands there when its registry's `navStaleAfter` has never been read — no deadline is invented. The policy is cached per chain for ten minutes and a failed refresh keeps the last value read, so a relay hiccup does not blank every deadline. |
| 2026-09-13 | **CHANGED** — Circle Arc testnet, chain `5042002`, is a supported chain (D-027 amendment). `?chainId=5042002` is accepted on every asset route and `meta.chainId` can read `5042002`; the `400` / `503 CHAIN_UNAVAILABLE` rules are unchanged. Arc has the same asset build as Hedera but its OWN addresses: one deployer at the same nonce put DIFFERENT contracts at the same address on the two chains. | Never key anything by contract address alone — carry the chain id with every address. A chain switcher reading addresses from one chain's config while `chainId` names another will show the wrong contract with no error. **Serving topology:** one API base URL serves both live chains. The process that indexes Hedera also serves Arc (it holds Arc's RPC for live reads), so a frontend needs one URL and selects the chain with `?chainId=`. The Arc indexer runs as its own process, one per chain (D-030), and is not a second API to call. After an Arc transaction, poll the same `/v1/health` and read Arc's progress from the `indexers[]` entry whose `chainId` is `5042002`. |
| 2026-09-13 | **CHANGED** — `/v1/health` gains `pendingBackfills`: one entry per chain (`chainId`, `count`) with discovered contracts whose logs from before their discovery are not fetched yet. Chains with none are absent. A non-zero count on the chain the process indexes moves `status` to `degraded`; other chains are reported without degrading it. | While a chain is listed, its data for recently discovered contracts is knowingly incomplete: do not present that chain's compliance, identity or floor state as complete. Seeing it for one poll while a backfill runs is normal; a count that persists means the backfill keeps failing and is worth raising. |
| 2026-09-13 | **CHANGED** — `/v1/health` gains `rollbacks`: one entry per chain that has ever rolled back, with `count` and `last` (`at`, `fromBlock`, `ancestorBlock`, `blocksDiscarded`, `logsDiscarded`, `cursorDeleted`), read from a durable record. Informational only: a genuine reorg is chain behaviour, so `rollbacks` never moves `status`. Absent chains have never rolled back. | Show it as an operational signal, not an error state. On Hedera (296), which has no reorgs, **any** entry is a defect worth raising. On a relay-fronted chain a count that climbs is worth reading before trusting the indexed data. `cursorDeleted: true` means that rollback forced a backfill from the start block. |
| 2026-09-12 | **CHANGED** (recorded late, 2026-09-13 — shipped in `ceda038` without this row) — `/v1/health` gains `risks`: one entry per asset on the indexed chain with `navExpiresAt` (nullable), `navStale`, and `lastPriceVsNavBps` (signed basis points, nullable when the asset has never traded). Divergence is measured from the most recent indexed hourly close, not a live pool read, and the field name says so. Since D-039 nothing on-chain reads `isNAVStale`, so this is the only control between a stale valuation and mispriced redemptions. | Warn as `navExpiresAt` approaches, not only once `navStale` flips. Never present `lastPriceVsNavBps` as live spot. |
| 2026-09-12 | **CHANGED** — `/v1/health` `anomalies` gains `indexedChain` and `byChain`. `open` still counts every chain in the shared database, but only `indexedChain` — the chain this process indexes — moves top-level `status` to `degraded`; another chain's stale cursor or backlog no longer does either. Also **CHANGED**: `/metrics` `twap` is `null` on every current deployment and its `windowSeconds` is gone (D-036: the manager publishes no time-weighted price, and returns 0 meaning "not published", which is never served as a price). `marketStatus` is keyed on the spot price alone; `warming_up` stays in the union as a reserved value that nothing returns today. `/positions` `currentTick` now comes from the pool's own `slot0` rather than the manager — same units, same shape, no migration. | A dashboard should read `anomalies.indexedChain` for "is this service unwell" and `byChain` for where a backlog actually is. Render no TWAP and describe nothing as an average. `currentTick` needs no change and is now correct: it previously served 0 on any deployment past D-036, while ranges sit near ±276,000. |
...
| 2026-09-13 | **CHANGED** — new `GET /v1/assets/:assetId/swap-quote?tokenIn=&amountIn=` (D-037). Quotes a `swapExactInput` trade before anyone signs: `tokenIn` must be this asset's token or this chain's stablecoin (whichever it is, the response infers `tokenOut`); `amountIn` is a decimal string in `tokenIn`'s own units. Response: `{ tokenIn, tokenOut, amountIn, spent, amountOut, partialFill }`, `provenance: "onchain"` always — it is a live `eth_call` simulating the real trade against the real pool (tick-crossing, partial fills, all of it), never a reimplementation of the math. **`spent` and `amountOut` are two different reads, not two names for the same call:** `swapExactInput`'s Solidity signature returns only `amountOut`; `spent` (the amount actually taken on a partial fill, per D-037's own refund behaviour) is recovered by an exact saturation test — a second call at double the input, and if the output is unchanged, a bounded binary search converges on `spent`. Costs one extra call on every quote and up to ~24 more, sequentially, on the (expected rare) saturated path — measured 1-2s for a normal quote and up to **~12s for a saturated one**, so show a distinct loading state for a large order rather than assuming the same latency as a small one. Backed by a wallet provisioned once per chain (verified, approved, funded with headroom for the doubling probe) whose ADDRESS is configuration and whose private key never touches this process — see `src/chain/quote.ts` and `docs-handover/PRODUCT_KNOWLEDGE.md` §7.5. **New error code:** `503 QUOTE_UNAVAILABLE` when no quote wallet is configured on this process. A revert the real trade would also hit (insufficient liquidity, wrong token, a paused market) comes back as `400 BAD_REQUEST` with `details.revertedWith` naming the Solidity error. | **Always compute effective price as `amountOut / spent`, never `amountOut / amountIn`** — they are identical when `partialFill` is false and diverge sharply when it is true (measured: a 50,000 mUSD request against thin depth spent only ~794.60, refunding the rest — using `amountIn` there would overstate the price by roughly 63×). Show `spent` and the refund explicitly whenever `partialFill` is true, exactly as the real trade's own event does. Treat a `400` as "size this down," not as a bug. Do not cache a quote past the moment it was fetched; pool state moves with every trade. `tokenOut` is derived, not configured — read it from the response rather than assuming which side is which. |
