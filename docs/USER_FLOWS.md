...
| 3.5 Deployment | Issuer | `keccak256(abi.encode(params)) == termsHash`; approved pool factory; non-zero identity registry | — | `deployAssetSystem` → `AssetSystemDeployed`, `AssetContractsSet`, `Active`; token bound to registry; pool + market manager exempt | implemented except hash check (D-026) |
